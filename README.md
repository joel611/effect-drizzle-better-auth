...
- `packages/db` — Drizzle schema, `drizzle-kit` config, and the `Db` Effect Layer (one `pg.Pool` + `drizzle-orm/node-postgres`, acquired and released as a scoped resource)
- `packages/core` — domain services built on top of `packages/db` (e.g. `TaskRepository`)
- `packages/auth` — Better Auth wrapped as Effect services (`Db` → `AuthAdapter` → `Auth`), using Better Auth's official `drizzleAdapter` over the same shared `Db` pool
